...
.PHONY: build network redis shell debug test lint setup tag untag testpypi pypi
...
redis: network
	-docker rm --force $(REDIS_HOST)
	docker run -d --network=$(NETWORK) -h $(REDIS_HOST) --name=$(REDIS_HOST) $(REDIS_IMAGE)
	docker run $(TTY) --rm --network=$(NETWORK) $(VOLUMES) $(ENVIRONMENT) $(ACCOUNT)/$(IMAGE):$(VERSION) sh -c "./redis.sh"
